export function getPaymentOptions(data, selectedCourses) {
  ...
  let payment_options = data?.payment_options
    ?.filter((option) => types.includes(option?.type))
  ...
  if (data?.payment_calculator?.type === "als_college") {
    payment_options = payment_options.filter((option) => {
      ...
      return (
        option?.code === "pay_upfront" ||
        selectedCourses.some((course) => {
          return parameters?.show_courses?.includes(
...
export function getPaymentOptionParameters(data, option, type) {
  return JSON.parse(
    data?.payment_options?.find(
      (item) => item?.code === option && item?.type === type
    )?.parameters
  );
}
...
export function getPaymentCalculatorDiscountPromotion(data, code) {
  const discountPromotion = data?.discount_promotions?.find(
    (dp) => dp.code === code
  );
  return {
    ...discountPromotion,
...
export function getCourseDiscountPromotion(data, courseCricosCode, code) {
  const discountPromotion = data?.course_pricings
    ?.find((cp) => cp?.course?.cricos_code === courseCricosCode)
    ?.discount_promotions?.find((dp) => dp.code === code);
  return {
    ...discountPromotion,
    parameters: JSON.parse(discountPromotion?.parameters),
  };
...
export function getSpecialCases(data, courses) {
  return [
    ...
    ...courses
      ?.map((course) => course?.coursePricing?.pricing_modifiers)
      .flat(),
  ].sort((a, b) => a.order - b.order);
}
